...
if __name__ == "__main__":  # 直接运行本文件时执行简易自测
    with BaseHttpClient(base_url="https://httpbin.org", timeout=15, max_retries=0) as client:  # 构造测试客户端并由with自动管理生命周期
        browser_headers = {  # 模拟浏览器的请求头
            "user-agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/152.0.0.0 Safari/537.36",  # 浏览器UA标识
            "accept-language": "zh-CN,zh;q=0.9"  # 首选语言
        }
        get_res = client.get("/get", params={"test_key": "test_value"}, headers=browser_headers)  # 发起GET自测
        print("GET Response:", get_res)  # 打印GET响应
        post_res = client.post("/post", json={"username": "demo", "pwd": "123456"}, headers=browser_headers)  # 发起POST自测
        print("POST Response:", post_res)  # 打印POST响应
