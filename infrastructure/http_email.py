...
from typing import Tuple, Dict, Any, Optional  # 类型标注:Tuple 用于返回值,其余供配置对象/可选参数扩展使用
...
class EmailNotifier:
    ...
    类常量(默认配置,可被实例构造参数覆盖):
        ...
        DEFAULT_SENDER_AUTH_CODE (str): 默认 SMTP 授权码(非登录密码);公开代码时需脱敏
    ...
    DEFAULT_SENDER_AUTH_CODE = "vfflavucdgyrcajh"  # 默认 SMTP 授权码(非登录密码);代码公开时需注意脱敏保护
    ...
    def send_text_email(self, to_email: str, subject: str, body: str) -> Tuple[bool, str]:
        ...
        server = None  # 先置空,保证连接建立前就失败时 finally 判空不会误操作
        try:
...
if __name__ == "__main__":  # 直接运行本文件时做模块自测;被 import 时不执行
    print("EmailNotifier 模块自测")
    notifier = EmailNotifier()  # 使用内置 QQ 邮箱默认配置实例化
    ok, msg = notifier.send_text_email(to_email="enghin110@qq.com", subject="模块自测", body="测试正文")  # 发送一封自测邮件验证链路
    print(f"send result ok={ok}, msg={msg}")  # 打印发送结果元组内容
