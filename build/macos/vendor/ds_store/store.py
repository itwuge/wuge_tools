...
codecs = {
    ...
    b"lsvP": PlistCodec,
    b"icvp": PlistCodec,
    # macOS 15 Finder 的 pBBk 值不是标准 bookmark, 解析会崩溃; 按原始 bytes 透传
...
class DSStoreEntry:
    ...
    def read(cls, block):
        ...
        # Finally, read the data (保留原始编码字节供 byte_length 使用)
        raw = None
        if typecode == b"bool":
            raw = block.read(1)
            ...
            vlen = block.read(b">I")[0]
            raw = block.read(2 * vlen)
            value = raw.decode("utf-16be")
        elif typecode == b"type":
            raw = block.read(4)
            value = raw
        elif typecode == b"comp" or typecode == b"dutc":
            raw = block.read(8)
            value = struct.unpack(b">Q", raw)[0]
        else:
            raise ValueError(f'Unknown type code "{typecode}"')

        entry = DSStoreEntry(filename, code, typecode, value)
        entry._raw_value = raw
        return entry
    ...
    def byte_length(self):
        ...
        # 读入的条目: 用原始编码字节算长度
        # (Finder 原生 plist blob 与 plistlib 重编码有字节差异,
        #  用重编码长度会导致 delete 删错字节数把 B-tree 写坏)
        raw = getattr(self, "_raw_value", None)
        if raw is not None:
            t = self.type if isinstance(self.type, bytes) else None
            if t == b"bool":
                return length + 1
            elif t == b"long" or t == b"shor":
                return length + 4
            else:
                return length + 4 + len(raw)
