        ...
        try:
            ...
            def _make_progress_cb(file_idx: int):
                ...
                def _cb(curr: int, total: int):
                    ...
                    if total > 0:
                        ...
                        # 进度单行刷新:界面日志区 + 控制台(打包版无控制台时界面仍可见)
                        if curr > 0:
                            _prog_text = f"⬇️ [{file_idx + 1}/{total_count}] 下载进度:{curr / 1048576:.1f}M/{total / 1048576:.1f}M"  # 字节换算为M显示  # 进度文本:已下载/总共
            ...
            try:
                ...
                for idx, asset_name in enumerate(self.download_items):
                    ...
                    # 下载失败的情况(含 sha256 校验不通过,损坏包不得继续安装)
                    if not dl_res.get("success") or not dl_res.get("sha256_ok", True):
                        # 如果是被取消导致的,直接返回(先结束进度行)
                        if self._cancel_event.is_set():
                            _console_write("\n")  # 进度行结束
                            return
                        # 否则抛出异常(先结束进度行)
