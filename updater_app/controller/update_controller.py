...
# 导入系统库, 用于平台判断(macOS DMG 安装时关闭窗口)
import sys
...
class UpdateAssistantController(QObject):
    ...
    def _on_asset_list(self, asset_list: list):
        ...
        """
        # 缓存资产列表,供后续匹配目标更新包使用
        self._asset_list = asset_list or []
    ...
    def _on_check_finished(self, res: dict):
        ...
        # 未找到目标更新包,尝试按平台前缀模糊匹配(兼容格式迁移: 旧版 tar.gz vs 新版 dmg)
        if not matched:
            # 提取平台前缀(去掉最后一个扩展名段), 如 "wuge_tools-macOS-arm64"
            # 兼容 .tar.gz / .dmg / .zip 等多段扩展名
            target_prefix = target_name
            for ext in (".tar.gz", ".tar.bz2", ".tar.xz", ".dmg", ".zip"):
                if target_prefix.lower().endswith(ext):
                    target_prefix = target_prefix[:-len(ext)]
                    break
            # 在资产列表中找同平台前缀的资产(忽略扩展名差异)
            fuzzy_match = None
            for item in self._asset_list:
                ...
                if name.startswith(target_prefix):
                    ...
                    break
            if fuzzy_match:
                target_name = fuzzy_match  # 用实际资产名替换目标名
                matched = True
                self.window.append_log(
                    f"ℹ️ 精确名未匹配,按平台前缀兼容匹配到:{fuzzy_match}"
                )
    ...
    def _start_install(self, package_path: str):
        ...
        # macOS DMG 安装: 关闭更新窗口, 后台静默挂载 .dmg 并替换 .app 包
        if sys.platform == "darwin" and package_path.lower().endswith(".dmg"):
            self.window.append_log("🪟 macOS DMG 更新: 关闭窗口, 后台挂载安装……")
            self.window.hide()
        # 连接日志信号 -> 窗口追加日志
    ...
    def _on_install_finished(self, result: dict):
        ...
        # 安装成功
        if ok:
            # Windows 安装版: 已启动 setup.exe,安装程序负责覆盖与启动新版本
            if result.get("setup_launched"):
                self.window.set_progress(100)
                self.window.set_status("已启动安装程序")
                self.window.append_log("🪟 已启动安装程序,完成后将自动启动新版本……")
                # 数据目录在 %LOCALAPPDATA%,不随安装目录覆盖,可直接回写版本号
