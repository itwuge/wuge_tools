...
def detect_app_dir() -> str:
    ...
    详细说明:
        ...
        onedir 模式下 update.exe 位于软件目录下的 update/ 子目录中,
        需回退到上一级作为软件目录,保证 app_dir 指向主程序所在目录。
    ...
    if getattr(sys, "frozen", False):
        ...
        # 检查当前目录名是否为 "update"(onedir 模式下 update.exe 在 update/ 子目录中)
...
def detect_data_store_root(app_dir: str, explicit: str = "") -> str:
    ...
    if pointed:
        return pointed
    # 第四优先级(兜底): 使用软件目录下的 data_store 子目录作为出厂默认
...
def detect_main_exe(app_dir: str, main_exe_name: str = "") -> str:
    ...
    详细说明:
        ...
        - 非 Windows: 取除 update 外的第一个无扩展名可执行文件
    ...
    for fname in entries:
        # 跳过更新助手自身(大小写不敏感比较)
        if fname in skip or fname.lower() in {s.lower() for s in skip}:
            continue
        # 拼接完整路径
        ...
        # 根据平台判断是否为可执行文件
        if exe_ext:
            # Windows 平台: 检查文件名是否以 .exe 结尾(大小写不敏感)
            if fname.lower().endswith(exe_ext):
                return fp
        else:
            # 非 Windows 平台: 取第一个无扩展名的可执行文件(排除隐藏文件)
            # 隐藏文件以 . 开头,有扩展名的文件包含 .
