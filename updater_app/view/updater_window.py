...
class UpdaterWindow(QMainWindow):
    ...
    def append_log(self, text: str):
        ...
        # 否则按普通日志追加新行。更新侧进度文本形如 "⬇️ [i/n] 下载进度:xxM/yyM",
        # 判断须兼容 [i/n] 前缀(不能只匹配 "⬇️ 下载进度:" 开头)
        if text.startswith("⬇️") and "下载进度:" in text:  # 判断是否为进度行文本
            doc = self.te_log.document()  # 取文档对象
            last_text = doc.lastBlock().text()
            if last_text.startswith("⬇️") and "下载进度:" in last_text:  # 最后一行已是进度行
                cursor = self.te_log.textCursor()  # 获取当前光标
                ...
                cursor.insertText(text)  # 用新进度文本替换该行,实现原地刷新
                return  # 完成替换,不再追加新行
        # 追加一行日志
