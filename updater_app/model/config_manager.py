...
BUILTIN_DEFAULTS: Dict[str, Dict[str, Any]] = {
    ...
    # 文件 IO 配置默认值
    "file_io": {
        "DOWNLOAD_CHUNK_SIZE": 1 * 1024 * 1024,   # 下载分片大小(1MB)
        "DOWNLOAD_MAX_WORKERS": 4,                 # 下载最大并发数
...
def render_update_save_filename(version_cfg: Dict[str, Any], tag: str = "") -> str:
    ...
    system = _pf.system()
    # 映射为统一的平台名
    plat = "Windows" if system == "Windows" else ("macOS" if system == "Darwin" else "Linux")
    # 从配置中获取文件名模板,默认值与 BUILTIN_DEFAULTS 保持一致
