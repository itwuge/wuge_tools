...
# 从基础设施层导入统一解压入口:Windows ZIP、Linux/macOS tar.gz
...
def install_update(
    ...
    参数:
        package_path (str): 更新包绝对路径(Windows .zip，Linux/macOS .tar.gz)
    ...
    # 按扩展名调用统一解压入口:Windows只使用ZIP，Linux/macOS只使用tar.gz
    ...
    try:
        ...
        if os.path.isfile(app_exe):
            # 如果备份文件已存在(上次异常残留),先删除
            if os.path.exists(backup_exe):
                _retry_io(lambda: os.remove(backup_exe))
            # 将旧主程序重命名为备份文件
            _retry_io(lambda: os.rename(app_exe, backup_exe))
            # 标记已完成重命名
            renamed = True
            # 记录备份日志
        ...
        if renamed:
            try:
                ...
                # 如果备份文件存在,改回原文件名
                if os.path.exists(backup_exe):
                    os.rename(backup_exe, app_exe)
                # 记录回滚成功日志
                log(f"↩️ 已回滚旧主程序:{os.path.basename(app_exe)}")
...
def launch_app(exe_path: str, app_dir: str, log_cb: LogCb = None) -> bool:
    ...
    try:
        ...
        if sys.platform.startswith("win"):
            ...
            # 非 Windows 平台: 使用 subprocess.Popen
            subprocess.Popen(
                [exe_path],           # 命令行参数列表
                cwd=app_dir,          # 工作目录
