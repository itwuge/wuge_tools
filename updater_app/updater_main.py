...
def _init_logger(app_dir: str) -> None:
    ...
    try:
        ...
        # 真正探测目录可写性:FileHandler 打开失败(目录存在但无写权限,
        #   如安装版 Program Files)时抛 OSError,统一走下方回退分支
        handler = logging.FileHandler(log_path, mode="w", encoding="utf-8")
    except OSError:
        # 目录创建失败或文件打开失败 权限不足等,回退到系统临时目录
        import tempfile

        # 使用系统临时目录存放日志(用户可写,安装版权限不足时兜底)
...
def _install_qt_zh_translation() -> None:
    ...
    try:  # 兜底:翻译文件缺失/加载失败时保持默认(不阻塞启动)
        from PySide6.QtCore import QLibraryInfo, QTranslator  # 延迟导入:仅此函数使用
        ...
        for _d in _cand_dirs:  # 逐个候选目录尝试
            if _d and _os.path.isdir(_d) and translator.load("qt_zh_CN", _d):  # 目录存在且加载成功
                ...
                break  # 停止尝试
        if _loaded:  # 加载成功才安装
            app.installTranslator(translator)  # 安装到应用实例,所有 Qt 组件生效
            app.setProperty("_qt_zh_translator", translator)  # 防止翻译器被垃圾回收
...
def main() -> int:
    ...
    _install_qt_zh_translation()  # 加载 Qt 内置中文翻译:文件对话框等系统组件显示中文
