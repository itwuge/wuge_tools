...
import sys  # 系统模块:按平台选择文件对话框实现(Windows原生/macOS自绘)
...
from PySide6.QtWidgets import QComboBox  # 下拉框控件,用于消息类型选择
...
from PySide6.QtWidgets import QScrollArea  # 滚动区控件,用于内容超出一屏时滚动查看
...
from PySide6.QtGui import QTextCursor  # 文本光标:定位/替换日志最后一行(进度单行刷新)
...
from runtime.view.theme import DARK_THEME_QSS  # 导入全局深色主题样式表(Windows/macOS 亮色模式下保持深色界面)
...
QGroupBox {
    ...
    background-color:#2a2a2b;               /* 背景:深灰色卡片(防系统亮色渗透) */
...
class MainWindow(QMainWindow):
    ...
    # 配置管理Tab - 发送邮件测试信号,无参数,触发后由Controller读取已保存配置发送
    sig_test_mail = Signal()
    # 配置管理Tab - 保存企业微信推送配置信号,参数为包含开关/Webhook链接/消息类型的字典
    sig_save_wecom = Signal(dict)
    # 配置管理Tab - 发送企业微信测试消息信号,无参数,触发后由Controller读取配置发送
    sig_test_wecom = Signal()
    ...
    # 文件传输Tab - 请求保存Tab内配置信号,无参数,点击"保存配置"按钮时发射
    sig_save_transfer_config = Signal()
    ...
    def __init__(self, local_version: str = ""):
        ...
        self.setStyleSheet(DARK_THEME_QSS)  # v1.0.8+: 应用全局深色主题(子控件自身样式优先覆盖)
        ...
        self.resize(780, 745)               # 设置窗口初始大小:宽780px,高745px
    ...
    def _build_tab_transfer(self):
        ...
        # 创建四个可编辑输入框:API基础地址、仓库所有者、仓库项目名、访问令牌
        ...
        # 可直接编辑:获取资产/下载时实时生效,点击"保存配置"可持久化到系统配置
        # (注意:GitHub配置为全局共享,保存后同时作用于自更新检测)
        ...
        # 创建四个可编辑输入框:下载分片、下载并发、上传分片、上传并发
        ...
        # 可直接编辑:下载/上传时实时生效,点击"保存配置"可持久化到系统配置
        self.le_dl_dl_chunk.setPlaceholderText("默认2097152(2MB)")     # 下载分片占位提示
        self.le_dl_dl_workers.setPlaceholderText("默认16")             # 下载并发占位提示(自适应上限)
        ...
        # 保存配置按钮:将本Tab内编辑的GitHub/文件IO配置写回系统配置并刷新
        self.btn_dl_save_cfg = QPushButton("💾 保存配置")     # 创建保存配置按钮
        self.btn_dl_save_cfg.setStyleSheet(PRIMARY_BTN_QSS)    # 应用主操作按钮样式(蓝色)
        self.btn_dl_save_cfg.clicked.connect(self._dl_click_save_config)  # 绑定点击槽函数
        left_layout.addWidget(self.btn_dl_save_cfg)     # 将保存配置按钮添加到左侧布局
        ...
        # =====** 保存目录行:可编辑输入框+浏览选择(与配置管理-路径配置同风格) =====**
        # 用户可直接在框内输入路径, 或点击"选择"浏览选取; 下载时以此框为准
        row_save_dir = QHBoxLayout()                     # 创建保存目录行水平布局
        row_save_dir.setSpacing(8)                       # 控件间距8px
        self.le_dl_save_dir = QLineEdit()                # 保存目录输入框(可编辑)
        self.le_dl_save_dir.setPlaceholderText("文件保存目录,可直接输入或点击[选择]浏览")  # 占位提示
        self.le_dl_save_dir.setFixedHeight(25)           # 固定高度25px与按钮对齐
        self.btn_dl_browse_dir = QPushButton("选择")     # 浏览选择目录按钮
        self.btn_dl_browse_dir.setFixedWidth(72)         # 固定宽度72px(与路径配置按钮一致)
        self.btn_dl_browse_dir.setFixedHeight(25)        # 固定高度25px
        self.btn_dl_browse_dir.clicked.connect(self._dl_click_browse_dir)  # 绑定浏览槽函数
        row_save_dir.addWidget(QLabel("保存目录:"))      # 添加标签
        row_save_dir.addWidget(self.le_dl_save_dir, stretch=1)  # 输入框占弹性空间
        row_save_dir.addWidget(self.btn_dl_browse_dir)   # 添加选择按钮
        right_layout.addLayout(row_save_dir)             # 将保存目录行添加到右侧布局
    ...
    def fill_transfer_github_config_view(self, gh_cfg: dict):
        """
        用文件传输Tab独立的GitHub配置回填左侧GitHub输入框

        由 Controller 在初始化或保存时调用,数据源为 transfer_github.json
        (下载器专用配置,与自更新github.json完全独立).

        :param gh_cfg: 下载器独立GitHub配置字典
        :type gh_cfg: dict
        :return: 无返回值
        :rtype: None
        """
        gh = gh_cfg or {}                                  # 空安全处理
        self.le_dl_gh_api_base_url.setText(str(           # 回填下载器API基础地址
            gh.get("GITHUB_API_BASE_URL", "https://api.github.com")  # 缺失时回退官方API地址
        ))
        self.le_dl_gh_owner.setText(str(gh.get("GITHUB_REPO_OWNER", "")))  # 回填仓库所有者
        self.le_dl_gh_name.setText(str(gh.get("GITHUB_REPO_NAME", "")))    # 回填仓库项目名
        self.le_dl_gh_token.setText(str(gh.get("GITHUB_TOKEN", "") or ""))  # 回填访问令牌
    ...
    def collect_github_config_view(self) -> dict:
        """收集文件传输Tab内GitHub配置输入框的当前值(供Controller合并生效/保存)."""
        return {
            "GITHUB_API_BASE_URL": self.le_dl_gh_api_base_url.text().strip(),
            "GITHUB_REPO_OWNER": self.le_dl_gh_owner.text().strip(),
            "GITHUB_REPO_NAME": self.le_dl_gh_name.text().strip(),
            "GITHUB_TOKEN": self.le_dl_gh_token.text().strip(),
        }

    def collect_fileio_config_view(self) -> dict:
        """收集文件传输Tab内下载器配置输入框的当前值(供Controller合并生效/保存)."""
        return {
            "DOWNLOAD_CHUNK_SIZE": self.le_dl_dl_chunk.text().strip(),
            "DOWNLOAD_MAX_WORKERS": self.le_dl_dl_workers.text().strip(),
            "UPLOAD_CHUNK_SIZE": self.le_dl_ul_chunk.text().strip(),
            "UPLOAD_MAX_WORKERS": self.le_dl_ul_workers.text().strip(),
        }
    ...
    def append_transfer_log(self, txt: str):
        ...
        :rtype: None
        """
        # 进度行单行刷新:若新增文本是进度且最后一行同为进度,原地替换该行(数字实时动,不刷屏);
        # 否则按普通日志追加新行
        if txt.startswith("⬇️ 下载进度:"):  # 判断是否为进度行文本
            doc = self.te_transfer_log.document()  # 取文档对象
            if doc.lastBlock().text().startswith("⬇️ 下载进度:"):  # 最后一行已是进度行
                cursor = self.te_transfer_log.textCursor()  # 获取当前光标
                cursor.movePosition(QTextCursor.MoveOperation.End)  # 移到文档末尾
                cursor.movePosition(QTextCursor.MoveOperation.StartOfBlock, QTextCursor.MoveMode.KeepAnchor)  # 选中到行首
                cursor.insertText(txt)  # 用新进度文本替换该行,实现原地刷新
                return  # 完成替换,不再追加新行
    ...
    def _dl_click_browse_dir(self):
        """
        点击保存目录[选择]按钮的处理函数(内部槽函数)

        弹出目录选择对话框, 将用户选择的路径回填到保存目录输入框;
        macOS 打包 app 原生对话框可能不响应, 强制 Qt 自绘(已加载中文翻译显示中文),
        Windows/Linux 用系统原生对话框(跟随系统语言).

        :return: 无返回值
        :rtype: None
        """
        current = self.le_dl_save_dir.text().strip()         # 读取输入框当前路径作为对话框初始目录
        if sys.platform == "darwin":  # macOS 平台
            picked = QFileDialog.getExistingDirectory(  # Qt 自绘目录对话框(避免原生对话框不响应)
                self, "选择文件保存目录", current,
                options=QFileDialog.Option.DontUseNativeDialog  # 自绘+中文翻译
            )
        else:  # Windows/Linux
            picked = QFileDialog.getExistingDirectory(self, "选择文件保存目录", current)  # 系统原生对话框
        if picked:                                           # 用户选择了目录
            self.le_dl_save_dir.setText(picked)              # 回填到保存目录输入框

    def fill_transfer_save_dir(self, dir_path: str):
        """
        回填文件传输Tab保存目录输入框的默认值(Controller初始化时调用).

        仅在输入框为空时回填, 不覆盖用户手动输入或浏览选择的路径.

        :param dir_path: 默认保存目录(通常为路径配置中的下载根目录)
        :type dir_path: str
        :return: 无返回值
        :rtype: None
        """
        if dir_path and not self.le_dl_save_dir.text().strip():  # 传入且当前为空才回填
            self.le_dl_save_dir.setText(str(dir_path))       # 回填默认保存目录
    ...
    def _dl_click_download(self):
        ...
        if not selected_names:                               # 如果没有勾选任何资产
            ...
            return                                            # 直接返回,不继续执行
        # 保存目录直接取输入框当前值(可编辑, 可先点[选择]浏览), 不再每次弹对话框
        save_dir = self.le_dl_save_dir.text().strip()        # 读取保存目录输入框当前值
        if not save_dir:                                     # 如果输入框为空
            self.show_warning("操作提示", "请先设置保存目录(直接输入路径或点击[选择]浏览)！")  # 警告提示
            return                                            # 直接返回,不继续执行
    ...
    def _dl_click_save_config(self):
        """
        点击"保存配置"按钮的处理函数(内部槽函数)

        将文件传输Tab内编辑的GitHub/下载器配置持久化到系统配置:
        1. 发射保存配置请求信号,交由Controller统一收集、校验并写盘
        2. 保存成功后Controller会回填刷新本Tab输入框

        :return: 无返回值
        :rtype: None
        """
        self.sig_save_transfer_config.emit()             # 发射保存配置请求信号

    # ======================================================================
    # 配置管理Tab 构建函数
    # 布局说明:紧凑两列布局,外层 QScrollArea 滚动区(窗口较小时内容可滚动查看,不裁切)
    # 左列:路径配置 + 邮件配置 + 企业微信推送
    ...
    def _build_tab_config(self):
        ...
        页面采用外层 QScrollArea 滚动区的左右两列紧凑布局:左列放路径、邮件与企业微信推送配置,
        右列放GitHub、下载及代理配置;窗口高度不足时出现滚动条,内容不被裁切;
        左右宽度按6:4分配,为路径输入框保留更多空间.
        ...
        outer = QVBoxLayout(self.tab_config)              # 创建页面最外层垂直布局
        outer.setContentsMargins(0, 0, 0, 0)              # 清除外边距(滚动区自含内边距)
        outer.setSpacing(0)                               # 滚动区紧贴页面
        self.scroll_config = QScrollArea()                # 创建滚动区:窗口高度不足时内容可滚动查看
        self.scroll_config.setWidgetResizable(True)       # 内容宽度自适应滚动区,避免横向滚动条
        self.scroll_config.setFrameShape(QScrollArea.Shape.NoFrame)  # 去掉滚动区边框,保持紧凑外观
        outer.addWidget(self.scroll_config)               # 滚动区铺满配置管理页
        content = QWidget()                               # 滚动内容容器
        self.scroll_config.setWidget(content)             # 将内容容器放入滚动区
        layout = QVBoxLayout(content)                     # 内容容器垂直布局
        layout.setContentsMargins(12, 12, 12, 12)         # 设置内容四周内边距12px
        layout.setSpacing(10)                             # 设置内容组件间距10px
        body = QHBoxLayout()                              # 创建左右双列主体布局
        body.setSpacing(10)                               # 设置左右列间距10px
        left = QVBoxLayout()                              # 创建左列布局:路径+邮件
        left.setSpacing(10)                               # 设置左列分组间距10px
        group_path, group_mail, group_wecom = self._build_path_mail_groups()  # 构建左列三个配置分组
        left.addWidget(group_path)                        # 添加路径配置分组
        left.addWidget(group_mail)                        # 添加邮件配置分组
        left.addWidget(group_wecom)                       # 添加企业微信推送配置分组
        ...
        layout.addLayout(body)                            # 将双列主体加入内容容器布局
    ...
    def _build_path_mail_groups(self):
        ...
        self.btn_test_mail = QPushButton("发送测试")      # 创建邮件发送测试按钮
        self.btn_reset_mail = QPushButton("重置")         # 创建仅清空邮件表单的重置按钮
        mail_btn_row.addWidget(self.btn_save_mail)        # 添加配置邮件按钮
        mail_btn_row.addWidget(self.btn_test_mail)        # 添加发送测试按钮
        mail_btn_row.addWidget(self.btn_reset_mail)       # 添加重置按钮
        lay_group_mail.addRow(mail_btn_wrap)              # 将按钮组添加到邮件表单底部

        # =====** 企业微信推送配置分组 =====**
        group_wecom = QGroupBox("企业微信推送")         # 创建企业微信推送配置分组框
        group_wecom.setStyleSheet(CARD_GROUP_QSS)         # 应用卡片式分组框统一样式
        lay_group_wecom = QFormLayout(group_wecom)        # 创建表单布局
        lay_group_wecom.setLabelAlignment(Qt.AlignmentFlag.AlignRight)  # 标签右对齐
        lay_group_wecom.setHorizontalSpacing(12)          # 水平间距12px
        lay_group_wecom.setVerticalSpacing(12)            # 垂直间距12px

        self.cb_enable_wecom = QCheckBox("启用企业微信推送")  # 启用企业微信推送复选框
        self.cb_enable_wecom.setToolTip("启用后,签到等任务结果会推送到企业微信群机器人")  # 提示信息
        lay_group_wecom.addRow(self.cb_enable_wecom)      # 添加复选框行(占两列)

        self.le_wecom_webhook = QLineEdit()               # 群机器人Webhook链接输入框
        self.le_wecom_webhook.setPlaceholderText("群机器人Webhook完整链接,含key=参数")  # 占位提示
        self.le_wecom_webhook.setEchoMode(QLineEdit.EchoMode.Password)  # 设置为密码模式(隐藏链接中的key)
        lay_group_wecom.addRow(QLabel("Webhook链接:"), self.le_wecom_webhook)  # 添加到表单布局

        self.cb_wecom_msg_type = QComboBox()              # 推送消息类型下拉框
        self.cb_wecom_msg_type.addItem("纯文本 (text)", "text")      # 最兼容,个人微信可显示
        self.cb_wecom_msg_type.addItem("Markdown", "markdown")       # 仅企业微信内渲染
        lay_group_wecom.addRow(QLabel("消息类型:"), self.cb_wecom_msg_type)  # 添加到表单布局

        # =====** 企业微信推送独立保存/重置/测试按钮行 =====**
        wecom_btn_wrap = QWidget()                        # 创建企业微信操作按钮包装容器
        wecom_btn_row = QHBoxLayout(wecom_btn_wrap)       # 创建按钮水平布局
        wecom_btn_row.setContentsMargins(0, 0, 0, 0)      # 清除包装容器内边距
        wecom_btn_row.addStretch(1)                       # 左侧留弹性空间,使按钮组右对齐
        self.btn_save_wecom = QPushButton("配置推送")    # 创建保存按钮
        self.btn_save_wecom.setStyleSheet(PRIMARY_BTN_QSS)  # 应用蓝色主操作按钮样式
        self.btn_test_wecom = QPushButton("发送测试")    # 创建发送测试按钮
        self.btn_reset_wecom = QPushButton("重置")       # 创建仅清空表单的重置按钮
        wecom_btn_row.addWidget(self.btn_save_wecom)      # 添加配置推送按钮
        wecom_btn_row.addWidget(self.btn_test_wecom)      # 添加发送测试按钮
        wecom_btn_row.addWidget(self.btn_reset_wecom)     # 添加重置按钮
        lay_group_wecom.addRow(wecom_btn_wrap)            # 将按钮组添加到企业微信表单底部

        return group_path, group_mail, group_wecom       # 返回三个分组框
    ...
    def _build_sys_groups(self):
        ...
        self.le_download_chunk.setPlaceholderText("下载分片大小(字节),默认2097152(2MB)")  # 占位提示
        lay_fileio.addRow(QLabel("下载分片大小:"), self.le_download_chunk)  # 添加到表单布局

        self.le_download_workers = QLineEdit()           # 下载并发数输入框
        self.le_download_workers.setPlaceholderText("下载最大并发线程数,默认16(自适应上限)")  # 占位提示
    ...
    def _build_tab3(self):
        ...
        group_summary.setStyleSheet("QGroupBox { font-weight: bold; font-size:14px; padding-top:14px; background-color:#2a2a2b; }")  # 分组框样式:加粗,14px字号
        ...
        group_acct.setStyleSheet("QGroupBox { font-weight: bold; font-size:14px; padding-top:14px; background-color:#2a2a2b; }")  # 分组框样式
        ...
        group_login.setStyleSheet("QGroupBox { font-weight: bold; font-size:14px; padding-top:14px; background-color:#2a2a2b; }")  # 分组框样式
        ...
        lay_group_login.setVerticalSpacing(9)            # 垂直间距9px
        ...
        group_detail.setStyleSheet("QGroupBox { font-weight: bold; font-size:14px; padding-top:14px; background-color:#2a2a2b; }")  # 分组框样式
        ...
        self.te_account_detail.setMinimumHeight(340)     # 固定高度350px,避免占用过多空间   
        lay_detail.addWidget(self.te_account_detail)     # 将详情文本框添加到布局
        right_layout.addWidget(group_detail, stretch=1)             # 将详情分组添加到右侧布局,占1份弹性空间

        # =====** 签到日志组件 =====**
        group_log = QGroupBox("签到日志")                # 创建签到日志分组框
        group_log.setStyleSheet("QGroupBox { font-weight: bold; font-size:14px; padding-top:14px; background-color:#2a2a2b; }")  # 分组框样式
        ...
        self.te_sign_log.setMaximumHeight(45)     # 最大高度45px,避免占用过多空间
        lay_log.addWidget(self.te_sign_log)              # 将日志文本框添加到布局
        right_layout.addWidget(group_log, stretch=1)     # 将日志分组添加到右侧布局,占1份弹性空间

        body_layout.addLayout(right_layout, stretch=7)   # 将右侧布局添加到主体布局,占7份弹性空间
    ...
    def _on_test_mail(self):
        """发射发送邮件测试信号,由Controller读取已保存配置并发送测试邮件.

        :return: 无返回值
        :rtype: None
        """
        self.sig_test_mail.emit()                        # 交给Controller发送测试邮件

    def _on_save_wecom(self):
        """收集企业微信推送表单并发射独立保存配置信号.

        :return: 无返回值
        :rtype: None
        """
        self.sig_save_wecom.emit({                        # 组装开关/链接/类型后交给Controller
            "enable_wecom": self.cb_enable_wecom.isChecked(),
            "webhook_url": self.le_wecom_webhook.text().strip(),
            "msg_type": self.cb_wecom_msg_type.currentData(),
        })

    def _on_reset_wecom(self):
        """重置企业微信推送配置输入区,不影响已落盘数据.

        :return: 无返回值
        :rtype: None
        """
        self.cb_enable_wecom.setChecked(False)             # 取消启用推送开关
        self.le_wecom_webhook.clear()                     # 清空Webhook链接输入框
        self.cb_wecom_msg_type.setCurrentIndex(0)         # 消息类型回到默认纯文本

    def _on_test_wecom(self):
        """发射发送测试消息信号,由Controller读取已保存配置并推送.

        :return: 无返回值
        :rtype: None
        """
        self.sig_test_wecom.emit()                        # 交给Controller发送测试消息
    ...
    def fill_form_data(self, data: dict):
        ...
        # =====** 回填企业微信推送配置 =====**
        wecom = data.get("wecom", {})                     # 获取企业微信推送配置字典
        self.cb_enable_wecom.setChecked(bool(wecom.get("enable_wecom", False)))    # 回填启用开关
        self.le_wecom_webhook.setText(str(wecom.get("webhook_url", "")))          # 回填Webhook链接
        msg_idx = self.cb_wecom_msg_type.findData(str(wecom.get("msg_type", "text")))  # 定位已保存的消息类型
        self.cb_wecom_msg_type.setCurrentIndex(msg_idx if msg_idx >= 0 else 0)       # 回填消息类型(未知回退纯文本)
    ...
    def _bind_signals(self):
        ...
        self.btn_save_wecom.clicked.connect(self._on_save_wecom)
        self.btn_test_wecom.clicked.connect(self._on_test_wecom)
        self.btn_reset_wecom.clicked.connect(self._on_reset_wecom)
