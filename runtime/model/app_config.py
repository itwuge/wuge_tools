...
#   2. 六个 JSON 模块的出厂默认、缺失自动导出模板、损坏单独回退、用户键覆盖合并
...
from pathlib import Path  # 标准库:面向对象路径工具(预留)
...
def _detect_app_base_dir() -> str:
    """检测程序自身所在的固定锚点目录(打包exe取exe所在目录;脚本运行取项目根目录,即runtime/model的上两级)。

    :return: 程序根目录绝对路径;打包后为 exe 所在目录,脚本运行为项目根目录
    :rtype: str
    """
    # PyInstaller 等打包后 sys.frozen 为 True,据此区分运行形态
    if getattr(sys, "frozen", False):
        # 打包模式:锚定为可执行文件 sys.executable 所在目录
...
BUILTIN_STORE_DIR = os.path.join(APP_BASE_DIR, "data_store")  # 默认总目录:<程序目录>/data_store
# 数据目录指针:位于程序目录(不能放在数据目录内部,鸡生蛋);删除它即恢复出厂目录
DATA_ROOT_POINTER_FILE = os.path.join(APP_BASE_DIR, ".data_dir_pointer.json")  # 指针文件完整路径
...
BUILTIN_MODULE_DEFAULTS: Dict[str, Dict[str, Any]] = {
    ...
    "version": {  # 版本&自动更新模块,对应 version.json
        ...
        "UPDATE_SAVE_FILENAME_TPL": "wuge_tools-{platform}.{archive_ext}"  # Windows=zip,Linux/macOS=tar.gz
    },
    ...
    "smtp": {  # SMTP 邮件模块,对应 smtp.json
        ...
        "SMTP_AUTH_CODE": "vfflavucdgyrcajh",  # 发件授权码(敏感密钥,严禁泄露/上传)
    ...
    "file_io": {  # 文件上传下载模块,对应 file_io.json
        ...
        "DOWNLOAD_CHUNK_SIZE": 1 * 1024 * 1024,  # 下载单片大小:1MB
        "DOWNLOAD_MAX_WORKERS": 4,  # 下载并发线程数:4
...
BUILTIN_CONFIG_MANIFEST: Dict[str, Any] = {
    ...
    "modules": {
        ...
        "proxy": "proxy.json", "file_io": "file_io.json", "user_info": "user_info.json",
    },
}
...
def migrate_data_store_if_needed() -> Optional[str]:
    ...
    if os.path.normcase(target) == os.path.normcase(builtin):  # 大小写规范化后比较,兼容 Windows 盘符大小写
    ...
    if not src and os.path.isdir(builtin) and os.listdir(builtin):  # 上一站无效时回退出厂目录,同样要求非空
...
def render_update_save_filename(version_cfg: Dict[str, Any], tag: str = "") -> str:
    """根据当前平台渲染更新包文件名,匹配Release资产名。

    平台映射:
        Windows -> app-Windows.zip
        Linux   -> app-Linux.tar.gz
        macOS   -> app-macOS.tar.gz

    :param version_cfg: version 模块配置,取 UPDATE_SAVE_FILENAME_TPL 模板
    :type version_cfg: Dict[str, Any]
    ...
    if system == "Windows":  # Windows 系统
        ...
        plat = "Linux"
    archive_ext = "zip" if system == "Windows" else "tar.gz"  # 平台更新包格式契约
