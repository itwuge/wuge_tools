...
def get_updater_path() -> Optional[str]:
    ...
    兼容两种打包布局:
        - onefile: 软件目录/update.exe
        - onedir:  软件目录/update/update.exe(PyInstaller onedir 产物放在 update/ 子目录)
    :return: 助手可执行文件绝对路径;源码运行或两种布局均缺失时返回 None
    :rtype: Optional[str]
    """
    ...
    return None  # 两种布局都没找到,判定助手未随包发布
