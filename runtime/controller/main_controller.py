...
class MainController:
    ...
    def _bind_signals(self):
        ...
        self.view.sig_test_mail.connect(self._on_test_mail)      # 邮件发送测试 -> 独立处理槽
        self.view.sig_save_wecom.connect(self._on_save_wecom)    # 企业微信推送保存 -> 独立处理槽
        self.view.sig_test_wecom.connect(self._on_test_wecom)    # 企业微信发送测试 -> 独立处理槽
    ...
    def _on_test_mail(self):
        """读取已保存的邮件配置并发送一封测试邮件验证链路.

        邮件配置不完整时给出明确提示;发送结果通过弹窗反馈
        (成功/失败原因),失败原因来自底层返回值.
        ...
        """
        try:
            mail_cfg = self.model.load_form_data().get("mail", {})  # 取已保存的邮件配置
            smtp_host = str(mail_cfg.get("smtp_host", "")).strip()      # SMTP服务器
            smtp_port = str(mail_cfg.get("smtp_port", "")).strip()      # SMTP端口
            sender_email = str(mail_cfg.get("sender_email", "")).strip()  # 发件邮箱
            smtp_code = str(mail_cfg.get("smtp_code", "")).strip()      # SMTP授权码
            receive_email = str(mail_cfg.get("receive_email", "")).strip()  # 接收邮箱
            # 校验必填项是否齐全
            if not all([smtp_host, smtp_port, sender_email, smtp_code, receive_email]):
                self.view.show_warning(
                    "发送测试",
                    "邮件配置不完整！\n请先填写 SMTP 服务器/端口/发件邮箱/授权码/接收邮箱并保存。",
                )
                return
            from service.send_email import ServiceEmailSender  # 函数内导入,避免顶层耦合
            sender = ServiceEmailSender(  # 构造业务邮件发送器
                smtp_host=smtp_host,  # 透传 SMTP 服务器
                smtp_port=int(smtp_port),  # 透传 SMTP 端口
                smtp_user=sender_email,  # 透传发件邮箱
                smtp_password=smtp_code,  # 透传授权码
                timeout=15,  # 发送超时15秒
            )
            subject = "签到软件 - 邮件发送测试"  # 测试邮件主题
            body = "✅ 邮件发送测试成功\n\n来自签到软件配置管理"  # 测试邮件正文
            ok, msg = sender.send_text(receive_email, subject, body)  # 发送测试邮件
        except Exception as e:  # 构造/发送过程中的任意异常
            _logger.error(f"❌ 邮件测试发送异常:{e}", exc_info=True)
            self.view.show_error("发送测试", f"测试发送异常:{e}")
            return
        # 按发送结果弹窗反馈
        if ok:  # 发送成功
            _logger.info("✅ 邮件测试发送成功")
            self.view.show_info("发送测试", f"测试邮件已发送到 {receive_email}！\n\n({msg})")
        else:  # 发送失败:明确提示失败原因
            _logger.warning(f"⚠ 邮件测试发送失败:{msg}")
            self.view.show_warning("发送测试", f"测试发送失败：\n{msg}")

    @Slot(dict)
    def _on_save_wecom(self, payload: dict):
        """校验企业微信 Webhook 地址并独立保存企业微信推送配置.

        启用推送时必须填写合法 Webhook 地址(指向企业微信官方接口且带 key 参数),
        未启用时允许留空保存(便于先配好开关,稍后再填链接).

        Args:
            payload (dict): 企业微信开关、Webhook 完整地址与消息类型.

        Returns:
            None
        """
        # 启用时强制校验 Webhook 地址必填且格式合法
        if bool(payload.get("enable_wecom", False)):
            webhook_url = str(payload.get("webhook_url", "")).strip()
            if not webhook_url:
                self.view.show_warning("输入校验", "启用企业微信推送时必须填写 Webhook 链接！")
                return
            from infrastructure.http_wecom import WeComWebhookNotifier  # 函数内导入,避免顶层耦合
            if not WeComWebhookNotifier.is_valid_webhook(webhook_url):
                self.view.show_warning(
                    "输入校验",
                    "Webhook 链接格式不正确！\n"
                    "应形如: https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=xxxx",
                )
                return
        try:
            self.model.save_wecom_config(payload)
            tip = "企业微信推送配置已保存！"
            if bool(payload.get("enable_wecom", False)):
                tip += "\n\n可点击「发送测试」验证推送链路是否打通。"
            self.view.show_info("保存成功", tip)
        except Exception as e:
            _logger.error(f"❌ 企业微信推送配置保存异常:{e}", exc_info=True)
            self.view.show_error("保存失败", f"企业微信推送配置保存异常:{e}")

    @Slot()
    def _on_test_wecom(self):
        """读取已保存的企业微信配置并发送一条测试消息验证链路.

        未启用推送或未配置 Webhook 地址时给出明确提示;
        发送结果通过弹窗反馈(成功/失败原因),失败原因来自底层返回值.

        Returns:
            None
        """
        try:
            wecom_cfg = self.model.load_form_data().get("wecom", {})  # 取已保存的企业微信配置
            webhook_url = str(wecom_cfg.get("webhook_url", "")).strip()  # Webhook 地址
            if not webhook_url:  # 未配置 Webhook 地址
                self.view.show_warning("发送测试", "尚未配置企业微信 Webhook 链接！\n请先填写链接并保存。")
                return
            from infrastructure.http_wecom import WeComWebhookNotifier  # 函数内导入,避免顶层耦合
            notifier = WeComWebhookNotifier(webhook_url)  # 构造底层推送器
            msg_type = str(wecom_cfg.get("msg_type", "text"))  # 消息类型
            if msg_type == "markdown":  # Markdown 测试消息
                content = "## ✅ 企业微信推送测试成功\n\n消息类型:**markdown**\n> 来自签到软件配置管理"
                ok, msg = notifier.send_markdown(content)  # 发送 Markdown 测试
            else:  # 默认纯文本测试消息
                content = "✅ 企业微信推送测试成功\n消息类型: text\n来自签到软件配置管理"
                ok, msg = notifier.send_text(content)  # 发送纯文本测试
        except Exception as e:  # 构造/发送过程中的任意异常
            _logger.error(f"❌ 企业微信测试推送异常:{e}", exc_info=True)
            self.view.show_error("发送测试", f"测试推送异常:{e}")
            return
        # 按发送结果弹窗反馈
        if ok:  # 发送成功
            _logger.info("✅ 企业微信测试推送成功")
            self.view.show_info("发送测试", f"测试消息已发送到企业微信群！\n\n({msg})")
        else:  # 发送失败:明确提示失败原因
            _logger.warning(f"⚠ 企业微信测试推送失败:{msg}")
            self.view.show_warning("发送测试", f"测试推送失败：\n{msg}")
