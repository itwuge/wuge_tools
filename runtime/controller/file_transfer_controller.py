...
class FileTransferController(QObject):
    ...
    def __init__(self, config: dict, main_view):
        ...
        self._bind_signals()
        # 加载文件传输Tab独立的GitHub配置(与自更新github.json分离)并回填展示
        self._transfer_github = self._load_transfer_github()
        self.view.fill_transfer_github_config_view(self._transfer_github)
        # 回填文件IO配置展示(下载器专用,file_io.json仅下载器使用)
        self.view.fill_fileio_config_view(self.config)
        # 回填保存目录默认值(路径配置中的下载根目录; 输入框为空才回填, 不覆盖用户输入)
        self.view.fill_transfer_save_dir(self._get_default_save_dir())
    ...
    def _get_default_save_dir(self) -> str:
        ...
        取路径配置中的下载根目录(DOWNLOAD_SAVE_ROOT); 获取失败时
        返回空串, 由用户在界面输入/浏览选择.

        :return: 默认保存目录绝对路径; 失败返回空串
        :rtype: str
        """
        try:
            from runtime.model.app_config import get_store_sub_paths  # 局部导入避免循环依赖
            return str(get_store_sub_paths()["DOWNLOAD_SAVE_ROOT"])  # 以界面配置的下载目录为准
        except Exception:
            return ""  # 获取失败返回空串, 界面保持空让用户自行选择
    ...
    def refresh_config(self, config: dict):
        ...
        # 更新本地配置引用(单一事实来源)
        self.config = config
        # 文件IO配置仅下载器使用,随全局配置刷新展示
        self.view.fill_fileio_config_view(self.config)
        # GitHub配置已独立(transfer_github.json),不随全局github.json刷新,
        # 避免系统配置Tab保存时覆盖下载器专用设置
    ...
    def _bind_signals(self):
        ...
        # 保存配置请求信号:用户点击"保存配置"按钮时触发
        # 连接到 _on_save_transfer_config 槽函数
        self.view.sig_save_transfer_config.connect(self._on_save_transfer_config)

    # ------------------------------------------------------------------
    # Tab内配置:合并生效 / 持久化保存
    # ------------------------------------------------------------------
    def _load_transfer_github(self) -> dict:
        """加载文件传输Tab独立的GitHub配置(transfer_github.json).

        与自更新github.json完全分离:首次使用(独立文件无仓库信息)时,
        从全局github.json拷贝一次作为初始默认值(不写盘);之后完全独立,
        不受系统配置Tab保存的影响.

        :return: 下载器GitHub配置字典
        :rtype: dict
        """
        try:
            # 懒导入独立配置读写函数,避免模块级循环依赖
            from runtime.model.app_config import load_transfer_github_config
            cfg = load_transfer_github_config() or {}   # 读取独立配置文件
            if cfg.get("GITHUB_REPO_OWNER") or cfg.get("GITHUB_REPO_NAME"):
                return cfg                              # 已有独立配置直接使用
        except Exception as e:                          # 读取失败回退空配置
            _logger.warning(f"⚠ 读取下载器GitHub独立配置失败:{e}")
            cfg = {}
        # 首次使用:从全局github.json拷贝一次作为初始默认(不写盘,保存后才落独立文件)
        base = (self.config or {}).get("github", {}) or {}
        return {
            "GITHUB_API_BASE_URL": base.get("GITHUB_API_BASE_URL") or "https://api.github.com",
            "GITHUB_REPO_OWNER": base.get("GITHUB_REPO_OWNER", ""),
            "GITHUB_REPO_NAME": base.get("GITHUB_REPO_NAME", ""),
            "GITHUB_TOKEN": base.get("GITHUB_TOKEN", ""),
        }

    def _merged_config(self) -> dict:
        """返回合并了文件传输Tab输入框当前值的配置副本(不改动全局配置).

        所见即所得:输入框当前值(含清空)直接覆盖对应键,拉取资产/下载/上传
        都以本方法返回的配置为准,保证"显示什么就用什么".

        :return: 合并后的配置字典副本
        :rtype: dict
        """
        cfg = dict(self.config)                        # 浅拷贝全局配置顶层
        github = dict(self._transfer_github or {})     # 以下载器独立GitHub配置为基准
        fileio = dict(cfg.get("file_io", {}) or {})  # 拷贝file_io节,避免污染原字典
        # 合并GitHub输入框当前值(空值也覆盖,与显示一致)
        github.update(self.view.collect_github_config_view())
        # 合并文件IO输入框当前值:优先转int,非法值/空值保留全局配置(worker永远拿到合法int)
        for key, val in self.view.collect_fileio_config_view().items():
            if val == "":
                continue                                # 空输入保留全局值
            try:
                fileio[key] = int(val)                  # 数字字符串转为整数
            except (TypeError, ValueError):
                continue                                # 非法数字保留全局值,避免异常流入Worker
        cfg["github"] = github                        # 写回合并后的github节
        cfg["file_io"] = fileio                       # 写回合并后的file_io节
        return cfg                                      # 返回合并副本

    @Slot()
    def _on_save_transfer_config(self):
        """保存文件传输Tab内编辑的GitHub/文件IO配置到磁盘并刷新展示.

        注意:GitHub仓库/令牌为全局共享配置,保存后同时作用于自更新检测;
        文件IO分片/并发保存后对所有下载上传生效.
        ...
        try:
            # 懒导入配置读写函数,避免模块级循环依赖
            from runtime.model.app_config import (
                load_and_merge_all_config,               # 重新加载全部配置
                save_file_io_config,                     # 保存文件IO配置
                save_transfer_github_config,             # 保存下载器独立GitHub配置
            )
            gh = self.view.collect_github_config_view()  # 收集GitHub输入框当前值
            save_transfer_github_config({                # 增量写盘transfer_github.json(独立于github.json)
                "GITHUB_API_BASE_URL": gh.get("GITHUB_API_BASE_URL") or "https://api.github.com",
                "GITHUB_REPO_OWNER": gh.get("GITHUB_REPO_OWNER", ""),
                "GITHUB_REPO_NAME": gh.get("GITHUB_REPO_NAME", ""),
                "GITHUB_TOKEN": gh.get("GITHUB_TOKEN", ""),
            })
            # 同步更新内存中的独立配置(下次拉取/下载立即生效)
            self._transfer_github = {
                "GITHUB_API_BASE_URL": gh.get("GITHUB_API_BASE_URL") or "https://api.github.com",
                "GITHUB_REPO_OWNER": gh.get("GITHUB_REPO_OWNER", ""),
                "GITHUB_REPO_NAME": gh.get("GITHUB_REPO_NAME", ""),
                "GITHUB_TOKEN": gh.get("GITHUB_TOKEN", ""),
            }
            # 文件IO默认值(与系统配置缺省一致;下载器已升级2MB/16,空值回退必须同源)
            defaults = {
                "DOWNLOAD_CHUNK_SIZE": 2 * 1024 * 1024,   # 下载分片缺省2MB(自适应分片,128片上限)
                "DOWNLOAD_MAX_WORKERS": 16,               # 下载并发缺省16(自适应爬升上限)
                "UPLOAD_CHUNK_SIZE": 5 * 1024 * 1024,     # 上传分片缺省5MB
                "UPLOAD_MAX_WORKERS": 3,                  # 上传并发缺省3
            }
            fi = {}                                        # 转换后的文件IO字典
            for key, val in self.view.collect_fileio_config_view().items():
                try:
                    fi[key] = int(val) if val else defaults[key]  # 空值回退默认
                except (TypeError, ValueError):
                    raise ValueError(f"{key} 必须为数字,当前值: {val!r}")  # 非法值报错
            save_file_io_config(fi)                        # 增量写盘file_io.json
            # 重新加载全局配置并刷新Tab展示(读入=写入,所见即所得)
            self.config = load_and_merge_all_config()      # 更新本地配置引用
            self.view.fill_transfer_github_config_view(self._transfer_github)  # 刷新独立GitHub配置展示
            self.view.fill_fileio_config_view(self.config) # 刷新文件IO配置展示
            _logger.info("💾 文件传输Tab配置已保存并刷新(独立于自更新)")   # 落盘记录
            self.view.show_info(
                "保存成功",
                "下载器配置已保存。\n获取资产、下载、上传将按新配置生效；\n下载器GitHub配置独立存储,不影响自更新检测。",
            )
        except Exception as e:                             # 保存失败兜底
            _logger.error(f"❌ 保存文件传输Tab配置失败:{e}")    # 落盘记录失败
            self.view.show_error("保存失败", f"配置保存失败:{e}")  # 弹窗提示
    ...
    def start_release_listing(self):
        ...
        # 创建Release资产列表拉取工作线程,传入合并了Tab输入框当前值的配置
        self.list_worker = ReleaseListWorker(self._merged_config())
    ...
    def _start_next_download(self):
        ...
        # 定义进度回调闭包:将单文件百分比转换为整体进度
        # pct: 当前文件已下载百分比(0~100,DownloadWorker 去抖后发出)
        ...
        # 创建下载工作线程,传入合并了Tab输入框当前值的配置和下载参数
        worker = DownloadWorker(
            config=self._merged_config(),         # 合并Tab输入框后的配置
    ...
    def _on_one_download_finished(self, res: dict):
        ...
        # 检查下载是否失败(含 sha256 校验不通过,损坏文件不得当成功)
        if not res.get("success") or not res.get("sha256_ok", True):
            # 下载失败:记录错误日志
    ...
    def _on_upload_requested(self, tus_endpoint: str, local_file_path: str):
        ...
        # 创建TUS上传工作线程,传入合并了Tab输入框当前值的配置和上传参数
        worker = UploadWorker(
            config=self._merged_config(),         # 合并Tab输入框后的配置
