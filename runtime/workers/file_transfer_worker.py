...
class DownloadWorker(BaseWorker):
    ...
    def run(self):
        ...
        try:  # 包裹整个下载流程，任何异常统一发射失败结果
            ...
            chunk_size = int(fileio.get("DOWNLOAD_CHUNK_SIZE", 1 * 1024 * 1024))  # 取下载分片大小，缺省为 1MB
            max_workers = int(fileio.get("DOWNLOAD_MAX_WORKERS", 4))  # 取下载并发线程数，缺省为 4
            ...
            timeout = int(github_cfg.get("GITHUB_TIMEOUT", 15))  # 取请求超时秒数，缺省为 15 秒
            github_token = str(github_cfg.get("GITHUB_TOKEN", "") or "").strip()  # 读取Token且不写入日志
            request_headers = {"Authorization": f"Bearer {github_token}"} if github_token else {}  # 有Token才附加认证头
            ...
            with ServiceFileDownloader(  # 构造下载器并使用 with 语句，保证退出时自动释放资源
                ...
                request_headers=request_headers,  # GitHub资产HEAD/GET请求携带Token认证头
            ) as dl:  # 下载器上下文实例
