...
from typing import Optional  # 标准库:类型注解,Optional 表示可空类型

from PySide6.QtWidgets import QApplication  # 第三方:PySide6 Qt 应用核心类,管理 GUI 应用的控制流与主设置
from runtime.view.theme import DARK_THEME_QSS  # 项目内:全局深色主题样式表,统一主窗口/对话框深色界面(不受系统亮暗影响)
...
class AppBootstrap:
    ...
    def init_logger(self) -> None:
        ...
        logging.basicConfig(  # 配置全局日志基础设置
            ...
            handlers=[  # 设置日志输出处理器列表(文件必写,控制台按有无 stdout 条件挂载)
                logging.FileHandler(log_path, mode="w", encoding="utf-8"),  # 文件处理器:写入日志文件,mode='w' 每次启动覆盖
                logging.StreamHandler(sys.stdout) if sys.stdout is not None else logging.NullHandler(),  # 控制台处理器:打包windowed无stdout时用空处理器,避免日志初始化崩溃
            ]
        )
    ...
    def create_qt_app(self) -> QApplication:
        ...
        # v1.0.8+: 应用全局深色主题(QApplication 级, 所有窗口/对话框统一深色, 不受系统亮暗影响)
        self.app.setStyleSheet(DARK_THEME_QSS)
        self._install_qt_zh_translation()  # 加载 Qt 内置中文翻译:文件对话框等系统组件显示中文
    ...
    @staticmethod
    def _install_qt_zh_translation() -> None:
        """加载 Qt 内置中文翻译(qt_zh_CN.qm),保证文件对话框等系统组件显示中文."""
        try:  # 兜底:翻译文件缺失/加载失败时保持默认(不阻塞启动)
            from PySide6.QtCore import QLibraryInfo, QTranslator  # 延迟导入:仅此方法使用
            translator = QTranslator()  # 翻译器实例
            import os as _os, sys as _sys  # 局部导入:候选翻译目录定位
            # 候选翻译目录:开发环境用 QLibraryInfo,打包环境兜底 _MEIPASS 下的 PySide6 translations
            _cand_dirs = [QLibraryInfo.path(QLibraryInfo.LibraryPath.TranslationsPath)]  # 开发/常规环境
            _mei = getattr(_sys, "_MEIPASS", None)  # PyInstaller 解包根目录
            if _mei:  # 打包环境
                _cand_dirs += [_os.path.join(_mei, "PySide6", "Qt", "translations"), _os.path.join(_mei, "translations")]  # 打包内翻译目录
            _loaded = False  # 是否成功加载标记
            for _d in _cand_dirs:  # 逐个候选目录尝试
                if _d and _os.path.isdir(_d) and translator.load("qt_zh_CN", _d):  # 目录存在且加载成功
                    _loaded = True  # 标记已加载
                    break  # 停止尝试
            if _loaded:  # 加载成功才安装
                from PySide6.QtWidgets import QApplication  # 延迟导入
                QApplication.instance().installTranslator(translator)  # 安装到应用实例,所有 Qt 组件生效
                # 防止翻译器被垃圾回收:挂到应用实例上
                QApplication.instance().setProperty("_qt_zh_translator", translator)
        except Exception:  # 任何异常(翻译缺失等)
            pass  # 静默忽略,界面仍可用(仅文件对话框为英文)
